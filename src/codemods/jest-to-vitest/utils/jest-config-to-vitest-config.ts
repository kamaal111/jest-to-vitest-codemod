...
function extractSetupFiles(configPairs: Array<SgNode<TypesMap>>): ReadonlyArray<string> | null {
...
function extractSnapshotSerializers(configPairs: Array<SgNode<TypesMap>>): ReadonlyArray<string> | null {
  const node = findValueNodeInPairs(configPairs, 'snapshotSerializers');
  if (node == null || isEmptyCollectionNode(node) || node.kind() !== 'array') return null;
...
export interface VitestConfigMapping {
  ...
  setupFiles?: ReadonlyArray<string> | null;
